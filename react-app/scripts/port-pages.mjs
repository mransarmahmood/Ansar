...
`;
  ...
  writeFileSync(resolve(OUT_DIR, `${pascal}.jsx`), out, 'utf8');
  generated.push({ slug, pascal, file });
}

// Emit a routes manifest for App.jsx to consume — uses React.lazy so each
// page ships as its own chunk (keeps the initial bundle small).
...
console.log(`Generated ${generated.length} pages (lazy-loaded chunks):`);
for (const g of generated) console.log('  /' + g.slug, '->', g.pascal + '.jsx');
