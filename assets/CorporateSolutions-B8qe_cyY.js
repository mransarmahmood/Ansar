  ...
  <section class="page-hero">
    ...
    <div class="container">
      ...
      <div class="page-hero__content">
        <span class="eyebrow eyebrow--white"><?= cms_e($sp['hero_eyebrow'] ?? 'Enterprise Partnership') ?></span>
        <h1><?= cms_e($sp['hero_headline'] ?? 'End-to-End Corporate HSE Solutions') ?></h1>
        <p><?= cms_e($sp['hero_description'] ?? 'Consulting, training, digital transformation, and AI — unified under one strategic partnership. From fractional HSE leadership to full-scale organisational transformation programs.') ?></p>
        <div class="page-hero__actions">
          <?php $cta1 = $sp['hero_cta_primary'] ?? []; $cta2 = $sp['hero_cta_secondary'] ?? []; ?>
          <a href="<?= cms_e($cta1['url'] ?? 'book-consultation.html') ?>" class="btn btn-gold btn-lg"><?php if (!empty($cta1['icon'])): ?><i class="<?= cms_e($cta1['icon']) ?>"></i> <?php endif; ?><?= cms_e($cta1['text'] ?? 'Discuss Partnership') ?></a>
          <a href="<?= cms_e($cta2['url'] ?? 'contact.html') ?>" class="btn btn-outline-white btn-lg"><?= cms_e($cta2['text'] ?? 'Enterprise Enquiry') ?></a>
        </div>
      </div>
    </div>
  </section>
...
`;function s(){return(0,i.useEffect)(()=>{document.title=`End-to-End Corporate HSE Solutions | Ansar Mahmood`},[]),(0,a.jsx)(r,{html:o})}export{s as default};